...
<nav aria-label="Page navigation example">
    <ul class="pagination d-flex justify-content-center">
        <li class="page-item <?= !isset($_GET['page']) || $_GET['page'] == 1 ? 'disabled' : null ?>">
            <a class="page-link" href="<?= BASE_URL . '/admin?action=prev' . $new_query_string ?>" aria-label="Previous">
        ...
        <?php if (isset($params['total_row'])) : ?>
            <?php for ($i = 1; $i <= $params['total_row']; $i++) : ?>
                <li class="page-item <?= (isset($_GET['page']) && $_GET['page'] == $i) || (!isset($_GET['page']) && $i == 1) ? 'active' : null ?>">
                    <a class="page-link" href="<?= BASE_URL . '/admin?page=' . $i ?>"><?= $i ?></a>
                </li>
            <?php endfor ?>
        <?php endif ?>
        <li class="page-item <?= (isset($_GET['page']) && $_GET['page'] >= $params['total_row']) || $params['total_row'] == 1 ? 'disabled' : null ?>">
            <a class="page-link" href="<?= BASE_URL . '/admin?action=next' . $new_query_string ?>" aria-label="Next">
